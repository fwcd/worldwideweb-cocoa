...
#endif
{
    ...
    if (cmd) {
        ...
        if (status < 0) {
            if (TRACE)
                printf("FTP: Error %d sending command", status);
            return status;
        }
    }
...
#endif
{
    ...
    if (TRACE)
        printf("FTP: Closing control socket %d\n", con->socket);
    if (connections == con) {
...

    {
        ...
        for (scan = connections; scan; scan = scan->next) {
            if (sin->sin_addr.s_addr == scan->addr) {
                if (TRACE)
                    printf("FTP: Already have connection for %d.%d.%d.%d.\n",
                           (int)*((unsigned char *)(&scan->addr) + 0), (int)*((unsigned char *)(&scan->addr) + 1),
                           (int)*((unsigned char *)(&scan->addr) + 2), (int)*((unsigned char *)(&scan->addr) + 3));
                return scan->socket; /* Good return */
            } else {
                if (TRACE)
                    printf("FTP: Existing connection is %d.%d.%d.%d\n", (int)*((unsigned char *)(&scan->addr) + 0),
                           (int)*((unsigned char *)(&scan->addr) + 1), (int)*((unsigned char *)(&scan->addr) + 2),
...
*/
    {
        ...
        if (status < 0) {
            ...
            if (TRACE)
                printf("FTP: Unable to connect to remote host for `%s'.\n", arg);
            NETCLOSE(con->socket);
            free(con);
...
*/
        {
            ...
            if (status == 2)
                status = response("USER anonymous\r\n");
            if (status == 3) {
            ...
            if (status != 2) {
                if (TRACE)
                    printf("FTP: Login fail: %s", response_text);
                NETCLOSE(con->socket);
                free(con);
                return -status; /* Bad return */
            }
            if (TRACE)
                printf("FTP: Logged in.\n");
        }
...
#endif
{
    ...
    if (TRACE)
        printf("FTP: Closed master socket %d\n", master_socket);
    master_socket = -1;
    if (status < 0)
...
*/
    ...
    if (TRACE)
        printf("FTP: Opened master socket number %d\n", new_socket);
...
*/
...
#ifdef POLL_PORTS
    {
        ...
        for (port_number = old_port_number + 1;; port_number++) {
            ...
            if ((status = bind(new_socket, (struct sockaddr *)&soc_address,
                ...
                break;
            if (TRACE)
                printf("TCP bind attempt to port %d yields %d, errno=%d\n", port_number, status, errno);
        } /* for */
    }
#else
    {
    ...
    CTRACE(tfp, "FTP: bound to port %d on %s\n", (unsigned int)ntohs(sin->sin_port), HTInetString(sin));
...
*/
    {
        ...
        if (status < 5) {
            if (TRACE)
                printf("FTP: PASV reply has no inet address!\n");
            return -99;
        }
...
*/
    {
        ...
        if (TRACE)
            printf("FTP: Data remote address is port %d, inet %d.%d.%d.%d\n", (unsigned int)ntohs(soc_address.sin_port),
        ...
        if (TRACE)
            printf("FTP data connected, socket %d\n", data_soc);
    }
#endif /* use PASV */
...
#endif
{
    ...
    if (soc != data_soc) {
        if (TRACE)
            printf("HTFTP: close socket %d: (not FTP data socket).\n", soc);
        return NETCLOSE(soc);
    }
    status = NETCLOSE(data_soc);
    if (TRACE)
        printf("FTP: Closing data socket %d\n", data_soc);
    if (status < 0)
...
#endif
{
    ...
    status = get_listen_socket();
    if (TRACE)
        printf("get_listen_socket returns %d\n\n", status);
    if (status < 0)
        exit(status);

    status = get_connection(argv[1]); /* test double use */
    if (TRACE)
        printf("get_connection(`%s') returned %d\n\n", argv[1], status);
    if (status < 0)
        exit(status);
#endif
    ...
    for (arg = 1; arg < argc; arg++) { /* For each argument */
        if (0 == strcmp(argv[arg], "-v")) {
            ...
            if (TRACE)
                printf("open_file_read returned %d\n\n", status);
            if (status < 0)
                exit(status);
	...
	*/
            ...
            if (TRACE)
                printf("Close_file returned %d\n\n", status);

        } /* if */
    } /* for */
    status = response("QUIT\r\n"); /* Be good */
    if (TRACE)
        printf("Quit returned %d\n", status);
