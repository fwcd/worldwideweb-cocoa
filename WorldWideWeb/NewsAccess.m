...
*/
    ...
    if (TRACE)
        printf("NewsAccess: Parsed address as port %4x, inet %d.%d.%d.%d\n", (unsigned int)ntohs(sin->sin_port),
...
static void get_styles() {
    ...
    if (!textStyle)
        textStyle = HTStyleNamed(styleSheet, "Example");
}
...
static int response(const char *command) {
    ...
    if (command) {
        ...
        if (status < 0) {
            if (TRACE)
                printf("NewsAccess: Unable to send comand. Disconnecting.\n");
            close(s);
            s = -1;
            return status;
        } /* if bad status */
        if (TRACE)
            printf("NNTP command sent: %s", command);
...
void read_group(const char *groupName, int first_required, int last_required) {
    ...
    if (TRACE)
        printf("Newsgroup status=%i, count=%i, (%i-%i) required:(%i-%i)\n", status, count, first, last, first_required,
               last_required);
    if (last == 0) {
...
- accessName:(const char *)arg anchor:(Anchor *)anAnchor diagnostic:(int)diag {
    ...
    if (TRACE)
        printf("NewsAccess: Looking for %s\n", arg);
    get_styles();
    {
        char *p1;
    ...
    for (retries = 0; retries < 2; retries++) {
        ...
        if (s < 0) {
            ...
            if (status < 0) {
                ...
                s = -1;
                if (TRACE)
                    printf("NewsAccess: Unable to connect to news host.\n");
                ...
                if (TRACE)
                    printf("NewsAccess: Connected to news host %s.\n", NewsHost);
