...
void output_paragraph(void) {
    ...
    int newlines = ((s->spaceBefore + s->spaceAfter) / s->paragraph.minimumLineHeight) + 1;
    int i;
...
void update_style(void) {
    ...
    if (current_style->free_format && cur && next) { /* generate new lines */
        int i;
        float space = cur->spaceAfter > next->spaceBefore ? cur->spaceAfter : next->spaceBefore; /* max */
        int newlines = (space / cur->paragraph.minimumLineHeight) + 1;
...
**	---------------------------------------------
*/
void change_style(SGML_style *s) { next_style = s; }
...
void end_style(void) {
    if (styleStack) {
        ...
        free(N);
        if (styleStack)
            change_style(styleStack->SGML);
        else
            change_style(&Normal);
    } else {
        if (TRACE)
            printf("HTML: We have ended more styles than we have started!\n");
...
void start_highlighting(HTStyle *style) { /* SET_STYLE(style);  @@@ to be fixed up */ }
...
static int parse_example(SGML_style *style, char *terminator) {
    ...
    for (;;) {
        if (END_OF_FILE)
            return S_text; /* return if end of stream */
        *q = NEXT_CHAR;
        if (upper(*q) == *p) {
            p++;
            q++;
...
*/
    ...
    while (!END_OF_FILE && (selectResult = select(fd + 1, &set, NULL, NULL, &timeout)) > 0 && (state != S_done)) {
        ...
        if (c == (char)-1) {
            if (TRACE)
                printf("*** HT: -1 found on input stream not at EOF!\n");
            break;
        }
...
*/
            ...
            rcdata:
                if (c == '&') { /* Entities */
                    ...
                    if (TRACE)
                        fprintf(stderr, "HTML: Bad entity.\n");
                    SETSTATE(S_word);
                }
            printable:
        ...
        case S_tag_start:
            ...
            case 'r':
            case 'R':
                SETSTATE(check("RESTOFFILE") ? S_restoffile : S_junk_tag)
            case 'T':
            case 't':
                SETSTATE(check("TITLE>") ? S_title : S_junk_tag);
            case 'U':
            case 'u':
                SETSTATE(S_tag_u);
            case 'X':
            case 'x':
                SETSTATE(check("XMP>") ? parse_example(&Example, "</XMP>") : S_junk_tag);
            case '/':
                SETSTATE(S_tag_end);
            default:
                SETSTATE(S_junk_tag);
        ...
        case S_tag_end:
            ...
            case 'n':
            case 'N':
                SETSTATE(check("NODE>") ? S_done : S_junk_tag)
            case 'O':
            case 'o':
                SETSTATE(S_end_o);
            case 'P':
        ...
        case S_tag_lis:
            SETSTATE(check("TING>") ? parse_example(&Listing, "</LISTING>") : S_junk_tag);
...
*/
        case S_tag_n:
            ...
            case 'o':
            case 'O':
                if (check("ODE>")) { /* Load a subnode */
                    if (TRACE)
                        printf("Loading subnode...NOT IMPLEMENTED\n");
#ifdef NOT_DEFINED
                    Anchor *a = [Anchor new];
                    HyperText *SN;
                ...
                if (check("EXTID ")) {
                    ...
                    for (;;) {
                        ...
                        if ((c == 'N') || (c == 'n')) {
                            if (!check("N = ")) {
                                if (TRACE)
                                    fprintf(stderr, "HTML: Bad nextid\n");
                                SETSTATE(S_junk_tag);
                            }
                            c = NEXT_CHAR;
                        }
...
*/
        case S_anchor:
            if (c == ' ')
                SETSTATE(S_anchor); /* Ignore spaces */
            if ((c == 'H') || (c == 'h')) {
                if (check("HREF = ")) {
                    SETSTATE(S_href);
                }
...
*/
    ...
    if ((state != S_text) && (state != S_done))
        if (TRACE)
            printf("*** Unfinished SGML file: Left in state %i\n", state);
...
*/
    while (styleStack) {
        ...
        styleStack = N->next;
        if (TRACE)
            printf("HT: Left in style at end of document!\n");
        free(N);
    }
...
SGML_style *findSGML(NSParagraphStyle *para) {
    ...
    if (TRACE)
        printf("HT: Can't find SGML style!\n");
    SGML_gen_errors++;
    return &Normal;
}
    ...
    if (r.anchor != last.anchor) { /* Start anchor */
        ...
        if (r.anchor) {
            ...
            if (d) {
                ...
                char *absolute = HTParse([n address], [[HT nodeAnchor] address], PARSE_ALL);
        ...
        while (chars_left) {
            ...
            if (prefix) {
                if (*prefix) {
                    ...
                    if (WHITE(c))
                        continue; /* Strip white space */
                    if (TRACE)
                        printf("HTML: WARNING: Paragraph prefix incomplete: %i found where %i expected.\n", c, *prefix);
                }
            ...
            if (c == '\n') { /* Paragraph Marks:	*/
                ...
                if (c == '\t') {
                    if (currentSGML)
                        NXPrintf(sgmlStream, "%s", currentSGML->tab_tag);
                    else
                        NXPrintf(sgmlStream, "\t");
                } else {                                                         /* Not tab or newline */
                    lineLength++;                                                /* @@bug doesn't count entity names */
                    if ((currentSGML->free_format) && (lineLength++ > LINE_WRAP) /* Wrap lines if we can */
                        && (c == ' ')) {
                        c = '\n';
                        lineLength = 0;
                    }
