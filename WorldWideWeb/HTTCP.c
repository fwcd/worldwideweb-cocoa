...
#endif
{
    ...
    sprintf(string, "%d.%d.%d.%d", (int)*((unsigned char *)(&HTHostAddress.sin_addr) + 0),
...
*/
    if (*host >= '0' && *host <= '9') {         /* Numeric node address: */
        ...
        if (!phost) {
            if (TRACE)
                printf("HTTPAccess: Can't find internet node name `%s'.\n", host);
            return -1; /* Fail? */
        }
    ...
    if (TRACE)
        printf("TCP: Parsed address as port %d, IP address %d.%d.%d.%d\n", (unsigned int)ntohs(sin->sin_port),
...

{
    ...
    if (TRACE)
        printf("     Name server says that is `%s' = %s\n", hostname, HTInetString(&HTHostAddress));
}
