...
#endif
{
    ...
    for (extension = filename + strlen(filename); (extension > filename) && (*extension != '.') && (*extension != '/');
         extension--) /* search */
        ;
...
#endif
{
    ...
    if (TRACE) {
        int i;
        printf("File mode is 0%o, uid=%d, gid=%d. My uid=%d, %d groups (", fileStatus.st_mode, fileStatus.st_uid,
...
*/
        if (fd < 0) {
            if (TRACE)
                printf("HTAccess: Can't open as %s\n", vmsname);
            sprintf(vmsname, "%s::\"%s\"", nodename, filename);
            fd = open(vmsname, O_RDONLY, 0);
            if (fd < 0) {
                if (TRACE)
...
*/
#ifndef NO_UNIX_IO
    {
        ...
        if (TRACE)
            printf("HTAccess: Opening `%s' gives %d\n", localname, fd);
        free(localname);
    }
#endif
#endif
